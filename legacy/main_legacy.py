...
from legacy.get_cex_price_legacy import BybitPrice, BingxPrice, BitfinexGeminiPrice, BitgetCoinwKucoinPrice, BitmexPrice, CexPrice, CryptocomPrice, DeribitPrice, DydxPrice, GarantexPrice, HuobiPrice, KrakenPrice, OkxPrice, PhemexPrice, PoloniexPrice, YoubitPrice
from legacy.get_dex_prices_legacy import DexscreenerAggregatorApi, ParaswapAggregatorApi, KyberswapAggregatorApi, OpenoceanAggregatorApi, OneInchAggregatorApi
...
def find_spread(exchanges, aggregators):
    ...
    if min_asks_exist and max_bids_exist and dex_price_exist:
        ...
        if percents_spread >= 3 and asks_amount >= 50 and bids_amount >= 50:
            print(f"**********SIGNAL**********")
            print(result_cex_output)
            with open('results/sync_signals.txt', 'a') as file:
        ...
        if dex_price > max_bid_value:
            ...
            with open('results/sync_results.txt', 'a') as file:
...
def cex_prices(src_token, dest_token):
        ...
        for network in networks:
            if src_token not in ["USDT", "USDC"]:
                if network.name in src_token["blockchains"]:
                    ...
                    aggregator_data = aggregator_object.get_price()
                    print(f"#{aggregator_data}")
                    if aggregator_data:
                        if "data" in aggregator_data:
                            ...
                            aggregator_price_info = {"aggregator": aggregator_object.name,
                                            ...
                                            "dex": aggregator_data["dex"],
                                            "data": ""}
                        print(f"!!!!!!!!!!!!!{aggregator_price_info}")
                        aggregator_price_list.append(aggregator_price_info)
                        print(f"Aggregator price list {aggregator_price_list}")
...
def main():
    ...
    for src_token in coins_info:
        ...
        exchanges = cex_prices(src_token, dest_token="USDT")
        print(f"!!!!!!!!!!!!!!!!!!!!!!!CEX{exchanges}")
        aggregators = dex_prices(src_token, dest_token="USDT")
        print(f"!!!!!!!!!!!!!!!!!!!!!!!DEX{aggregators}")
        find_spread(exchanges, aggregators)
        print("***************************")
        print(f"USDC - {src_token}")
        exchanges = cex_prices(src_token, dest_token="USDC")
        print(f"!!!!!!!!!!!!!!!!!!!!!!!CEX{exchanges}")
        aggregators = dex_prices(src_token, dest_token="USDC")
        print(f"!!!!!!!!!!!!!!!!!!!!!!!DEX{aggregators}")
        find_spread(exchanges, aggregators)
